...
<task type="auto">
  <name>Task 2: Add Log Category Export Macro</name>
  <files>
    Plugins/GSD_Crowds/Source/GSD_Crowds/Public/GSDCrowdLog.h
  </files>
  <action>
Add GSD_CROWDS_API export macro to log category declaration.
...
// CRITICAL: Must have GSD_CROWDS_API for cross-plugin symbol export
GSD_CROWDS_API DECLARE_LOG_CATEGORY_EXTERN(LogGSDCrowds, Log, All);
```

**Why This Matters (Architecture Rick Feedback):**
- Without GSD_CROWDS_API on DECLARE_LOG_CATEGORY_EXTERN, the log category symbol
  is not exported from the module's DLL
- Other plugins trying to use UE_LOG(LogGSDCrowds, ...) get linker errors
- The API macro must come BEFORE DECLARE_LOG_CATEGORY_EXTERN
...
// INCORRECT (no export):
DECLARE_LOG_CATEGORY_EXTERN(LogGSDCrowds, Log, All);
```
  </action>
  <verify>
    ```bash
    # Verify log category has API macro
    grep -q "GSD_CROWDS_API DECLARE_LOG_CATEGORY_EXTERN" Plugins/GSD_Crowds/Source/GSD_Crowds/Public/GSDCrowdLog.h && echo "✓ Log category has API macro" || echo "✗ Log category missing API macro"

    echo "Log category verification complete"
    ```
  </verify>
  <done>
    - GSDCrowdLog.h has GSD_CROWDS_API on log category
    - Cross-plugin logging works without linker errors
    - Log category properly exported from module
  </done>
</task>

<task type="auto">
  <name>Task 3: Add GSD_VEHICLES_API to All Public Classes</name>
  <files>
    Plugins/GSD_Vehicles/Source/GSD_Vehicles/Public/GSD_Vehicles.h
    Plugins/GSD_Vehicles/Source/GSD_Vehicles/Public/Actors/GSDVehiclePawn.h
    Plugins/GSD_Vehicles/Source/GSD_Vehicles/Public/Actors/GSDVehicleTestbedActor.h
