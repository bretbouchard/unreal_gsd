...
files_modified:
  ...
  - Plugins/GSD_Crowds/Source/GSD_Crowds/Public/DataAssets/GSDHeroNPCArchetype.h
  - Plugins/GSD_Crowds/Source/GSD_Crowds/Private/DataAssets/GSDHeroNPCArchetype.cpp
autonomous: true
must_haves:
  truths:
...
<task type="auto">
  <name>Task 3: Create UGSDHeroNPCArchetype DataAsset</name>
  <files>
    Plugins/GSD_Crowds/Source/GSD_Crowds/Public/DataAssets/GSDHeroNPCArchetype.h
    Plugins/GSD_Crowds/Source/GSD_Crowds/Private/DataAssets/GSDHeroNPCArchetype.cpp
  </files>
  <action>
Create UGSDHeroNPCArchetype DataAsset for Hero NPC character configuration (Swarm Rick Feedback).

**Purpose:** Hero NPCs need a configurable archetype for gameplay stats, visual appearance, and behavior parameters. This allows designers to create different Hero NPC types (Survivor, Soldier, Medic, etc.) without code changes.

**GSDHeroNPCArchetype.h:**
```cpp
#pragma once
...
**Important:**
- Separates visual, stats, AI config into one archetype
- Designers can create new NPC types without code changes
- ArchetypeTag for lookup by gameplay tag
- ValidateConfig for asset validation
- Soft references for async loading support
  </action>
  <verify>
    ```bash
    # Verify archetype file exists
    test -f Plugins/GSD_Crowds/Source/GSD_Crowds/Public/DataAssets/GSDHeroNPCArchetype.h && echo "✓ Hero NPC Archetype header exists" || echo "✗ Hero NPC Archetype header missing"

    # Check for required properties
    grep -q "MaxHealth" Plugins/GSD_Crowds/Source/GSD_Crowds/Public/DataAssets/GSDHeroNPCArchetype.h && echo "✓ MaxHealth defined" || echo "✗ MaxHealth missing"
    grep -q "AttackDamage" Plugins/GSD_Crowds/Source/GSD_Crowds/Public/DataAssets/GSDHeroNPCArchetype.h && echo "✓ AttackDamage defined" || echo "✗ AttackDamage missing"
    grep -q "SkeletalMesh" Plugins/GSD_Crowds/Source/GSD_Crowds/Public/DataAssets/GSDHeroNPCArchetype.h && echo "✓ SkeletalMesh defined" || echo "✗ SkeletalMesh missing"

    echo "Hero NPC Archetype verification complete"
    ```
  </verify>
  <done>
    - UGSDHeroNPCArchetype DataAsset created
    - Visual, stats, AI configuration in one asset
    - ValidateConfig for asset validation
    - ArchetypeTag for lookup
    - Supports designer-created NPC types
  </done>
</task>
...
5. UGSDHeroNPCArchetype created with visual, stats, AI config
6. Controller uses config for all settings
</verification>
...
- UGSDHeroNPCArchetype with visual, stats, AI configuration
- Blackboard updates on perception changes
</success_criteria>
