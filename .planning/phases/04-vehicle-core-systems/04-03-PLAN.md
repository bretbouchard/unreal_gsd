...
must_haves:
  ...
  key_links:
    ...
    - from: "GSDVehiclePawn"
      ...
      via: "SpawnFromConfig parameter"
      pattern: "SpawnFromConfig.*UGSDDataAsset"
    - from: "GSDVehiclePawn"
...
2. Create GSDVehiclePawn.h:
   ...
   - virtual void SpawnFromConfig(UGSDDataAsset* Config) override;  // Council: Renamed from SpawnAsync - uses sync loading
...
3. Implement SpawnFromConfig(UGSDDataAsset* Config):
...
4. Implement ApplyVehicleConfig(UGSDVehicleConfig* Config):
   - NULL CHECK (Council): If Config is null, log error and return early
   - NULL CHECK (Council): If GetMesh() is null, log error and return early
   - NULL CHECK (Council): If GetVehicleMovementComponent() is null, log error and return early

   - Load skeletal mesh: Config->VehicleMesh.LoadSynchronous()
   - NULL CHECK (Council): If mesh loaded successfully, set on GetMesh()
   - Load physics asset: Config->PhysicsAsset.LoadSynchronous()
   - NULL CHECK (Council): If physics asset loaded, set on GetMesh()
   ...
   - Configure wheel setups:
     ...
     - For each wheel config:
       - Load wheel config: Config->WheelConfigs[i].LoadSynchronous()
       - NULL CHECK (Council): If wheel config is null, skip and log warning
       - WHEEL VALIDATION (Council): Validate wheel bone exists in skeletal mesh before setup
       - Set WheelClass = UChaosVehicleWheel::StaticClass()
       - Set BoneName from wheel config

   - WHEEL PHYSICS PROPERTIES (Council - CRITICAL): Apply wheel config properties to each wheel:
     - After wheel setup, iterate through created wheels
     - For each wheel, apply from GSDWheelConfig:
       - WheelRadius, WheelWidth
       - SuspensionMaxRaise, SuspensionMaxDrop, SuspensionDampingRatio
       - FrictionForceMultiplier
       - MaxSteerAngle (if bAffectedBySteering)
       - bAffectedByHandbrake, bAffectedByEngine flags

   - Set engine torque curve if TorqueCurve is valid
...
CRITICAL: Use UChaosWheeledVehicleMovementComponent, NOT UWheeledVehicleMovementComponent (PhysX).
  ...
  <verify>
grep "AGSDVehiclePawn::SpawnFromConfig" Plugins/GSD_Vehicles/Source/GSD_Vehicles/Private/Actors/GSDVehiclePawn.cpp
...
grep "ConfigureForVehicle" Plugins/GSD_Vehicles/Source/GSD_Vehicles/Private/Actors/GSDVehiclePawn.cpp
  ...
  <done>
GSDVehiclePawn implementation with SpawnFromConfig, ApplyVehicleConfig (with wheel physics), and IGSDSpawnable methods.
</done>
</task>
