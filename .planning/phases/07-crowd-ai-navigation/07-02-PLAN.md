...
must_haves:
  truths:
    ...
    - "Entities have randomized movement speed to prevent synchronized movement"
  artifacts:
    ...
    - path: "Plugins/GSD_Crowds/Source/GSD_Crowds/Public/DataAssets/GSDCrowdEntityConfig.h"
      provides: "Velocity randomization configuration"
      contains: "VelocityRandomizationPercent"
  key_links:
    ...
    - from: "GSDNavigationProcessor"
      ...
      via: "Velocity randomization"
      pattern: "MovementSpeed"
---

<objective>
Create navigation processor that moves entities along ZoneGraph lanes with fallback to direct movement when ZoneGraph is unavailable. Implements velocity randomization to prevent synchronized movement (CROWD-08).

Purpose: Implement the core navigation logic for crowd entities. The processor queries ZoneGraphSubsystem for lane data, updates entity positions along lanes, and gracefully falls back to direct movement if ZoneGraph is unavailable (experimental status handling). Applies velocity randomization to create natural-looking crowd movement.

Output: UGSDNavigationProcessor that handles lane finding, lane following, fallback movement, and velocity randomization.
</objective>
...
6. Velocity randomization helper for CROWD-08
</action>
  <verify>
...
grep -q "ApplyVelocityRandomization" Plugins/GSD_Crowds/Source/GSD_Crowds/Public/Processors/GSDNavigationProcessor.h && \
echo "Navigation processor header created"
  </verify>
  <done>UGSDNavigationProcessor class defined with Execute(), ConfigureQueries(), helper methods, and velocity randomization</done>
</task>
...
<task type="auto">
  ...
  <action>
Implement GSDNavigationProcessor.cpp with ZoneGraph lane following logic and velocity randomization.
...
6. Velocity randomization (CROWD-08) applied to both lane and fallback movement
</action>
  <verify>
...
grep -q "ApplyVelocityRandomization" Plugins/GSD_Crowds/Source/GSD_Crowds/Private/Processors/GSDNavigationProcessor.cpp && \
echo "Navigation processor implemented with velocity randomization"
  </verify>
  <done>UGSDNavigationProcessor moves entities along ZoneGraph lanes with fallback to direct movement and velocity randomization (CROWD-08)</done>
</task>
...
These fields allow per-config navigation tuning. The processor will read these values when spawning entities. VelocityRandomizationPercent controls the +/- percentage variation applied to movement speed.
</action>
  <verify>
...
grep -q "VelocityRandomizationPercent" Plugins/GSD_Crowds/Source/GSD_Crowds/Public/DataAssets/GSDCrowdEntityConfig.h && \
echo "Entity config updated with navigation and velocity randomization fields"
  </verify>
  <done>GSDCrowdEntityConfig has navigation configuration fields (bEnableNavigation, DefaultMoveSpeed, LaneSearchRadius, VelocityRandomizationPercent)</done>
</task>
...
5. Velocity randomization implemented (CROWD-08)
6. Entity config has navigation and velocity randomization fields
</verification>
...
- Velocity randomization prevents synchronized movement (CROWD-08)
</success_criteria>
