...
must_haves:
  truths:
    ...
    - "Smart Objects are released after interaction completes"
  artifacts:
  ...
  key_links:
    ...
    - from: "GSDSmartObjectProcessor"
      ...
      via: "Claim/Release"
      pattern: "ClaimHandle"
    - from: "GSDSmartObjectProcessor.Execute"
      to: "ReleaseSmartObject"
      via: "Interaction completion check"
      pattern: "bInteractionComplete"
---

<objective>
Create Smart Object system (fragment, subsystem, processor) for world interaction points like cover, ambush, and wander points.

Purpose: Enable crowd entities to interact with Smart Objects in the world. The fragment stores interaction state, the subsystem wraps USmartObjectSubsystem for queries and claims, and the processor handles the claim/interact/release lifecycle with proper cleanup on interaction completion.
...
CRITICAL: The Execute() loop now calls ReleaseSmartObject() when bInteractionComplete is true, ensuring Smart Objects are properly released after interaction completes.
</action>
  <verify>
...
grep -A5 "bInteractionComplete" Plugins/GSD_Crowds/Source/GSD_Crowds/Private/Processors/GSDSmartObjectProcessor.cpp | grep -q "ReleaseSmartObject" && \
echo "Smart Object processor created with release on completion"
  </verify>
  <done>UGSDSmartObjectProcessor handles search, claim, interact, release lifecycle with proper cleanup on interaction completion</done>
</task>
...
6. Smart Objects are released when interaction completes (key link)
</verification>
...
- Release called after interaction completes
</success_criteria>
