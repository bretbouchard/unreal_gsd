...
- `RandomSeed` (int32, default 42, meta = (EditCondition = "bUseDeterministicSeed")) - Seed for reproducible spawns
- `bUseDeterministicSeed` (bool, default true) - Whether to use seeded randomization
- `TargetFPS` (float, default 60.0f) - Target FPS for performance validation
- `bLogPerformanceWarnings` (bool, default true) - Log warning if FPS drops below target
...
- `GetAverageFPS() const` - Get average FPS (1.0f / GetAverageFrameTime())
...
5. If bLogPerformanceWarnings and CurrentFPS < TargetFPS:
   ```cpp
...
2. Initialize random stream with seed for deterministic spawns:
   ```cpp
   ...
   ```
3. Get PoolSubsystem from GetWorld()->GetSubsystem<UGSDVehiclePoolSubsystem>()
4. Get SpawnerSubsystem from GetWorld()->GetSubsystem<UGSDVehicleSpawnerSubsystem>()
5. If bUsePooling and PoolSubsystem:
   - For each config in VehicleConfigs:
     - Load config synchronously
     - Call PoolSubsystem->WarmUpPool(Config, PoolSizePerConfig)
6. For i from 0 to NumVehiclesToSpawn:
   - Random config index: RandomStream.RandRange(0, VehicleConfigs.Num() - 1)
   - Load config synchronously
   - Random position within SpawnAreaSize:
     ```cpp
     ...
     ```
   - Random rotation Yaw: RandomStream.FRandRange(0.0f, 360.0f)
...
7. Log spawn count via GSD_VEHICLE_LOG
...
**GetAverageFPS:**
1. float AvgFrameTime = GetAverageFrameTime()
2. If AvgFrameTime > 0, return 1.0f / AvgFrameTime
3. Else return 0.0f
