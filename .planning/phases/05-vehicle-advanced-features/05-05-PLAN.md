...
Implement GetActiveTuningPreset (return ActiveTuningPreset).
...
In Initialize(), get pool subsystem reference:
```cpp
...
AGSDVehiclePawn* UGSDVehicleSpawnerSubsystem::SpawnVehicleFromPool(UGSDVehicleConfig* Config, FVector Location, FRotator Rotation)
{
    if (PoolSubsystem)
    {
        AGSDVehiclePawn* Vehicle = PoolSubsystem->AcquireVehicle(Config, Location, Rotation);
        if (Vehicle)
        {
